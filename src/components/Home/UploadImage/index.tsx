...
const UploadImage = ({
  ...
  const handleChange = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setPalletteColor(URL.createObjectURL(target.files[0]));
  };

  const onDrop = React.useCallback((acceptedFiles: (Blob | MediaSource)[]) => {
    setUploadImage(URL.createObjectURL(acceptedFiles[0]));
    setPalletteColor(URL.createObjectURL(acceptedFiles[0]));
  }, []);
